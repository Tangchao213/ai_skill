...
7. **按「功能单元」空行**（详见第 8 节）：一行就能完成的功能，写完空一行；多行才能
   完成的功能，内部不空行、整段之后再空行。
...
- **空行按「功能单元」分组**，不是按行机械空行：一行即完成的功能之后空一行；多行
  才能完成的功能内部不空行。详见第 8 节。
...
- **单行代码的注释用 `//`。** 给某一行（或某条语句）加说明，一律用 `//`，
  **不要**用 `/* ... */` 去注释单行。
- **`/* ... */` 只用于一整段 / 一个函数级别的代码。** 描述一个函数、一大段逻辑
  时才用块注释 `/* ... */`。
- 空白区段里放 `/*  */` 占位是可以的。
- 注释要解释**为什么**，而不是显而易见的**做了什么**。

## 8. 空行与功能分组（按 feature 分组）

用空行按「功能单元」来分组代码，而**不是**按单条语句机械地空行：

- **一行就能完成一个功能** → 写完这一行后**空一行**，再写下一个功能。例如 `main` 里
  `init` / 创建节点 / `spin` / `shutdown` 各是一行独立功能，彼此之间都空一行。
- **需要多行才能完成一个功能**（一个 `if` 块、一组紧密相关的连续语句等） → 这几行
  之间**不空行**，让它们成为一个紧凑的整体；整个功能块结束后再空一行，与下一个功能
  分隔。

一句话：**空行分隔的是「功能」，不是「行」。** 功能内部保持紧凑，功能与功能之间用
空行隔开。

```cpp
void TopicSubscribe01::command_callback(const std_msgs::msg::String::SharedPtr msg)
{
    double speed = 0.0f;

    // 多行才能完成的一个功能：if 块内部不空行，作为一个整体
    if (msg->data == "forward")
    {
        speed = 0.2f;
    }
